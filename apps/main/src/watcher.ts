...
export function startWatcher(): void {
  ...
  const pathToRecipe = new Map(watched.map(w => [w.path, w.recipe]));
...
export function addWatchedPath(watchPath: string, recipe: string): void {
  // For simplicity, restart watcher logic is handled by the API telling clients
  // the path was added. A full restart adds it to chokidar.
  // In production, maintain a single watcher instance and call watcher.add().
}
