...
import { Radio, LayoutDashboard, ListVideo, Cpu, Film, Settings, Zap, Menu } from 'lucide-react';
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  // Mobile drawer state — Sidebar already supports `mobileOpen`/`onClose`,
  // it just was never wired here. Without state + hamburger button, the bottom
  // tab bar (5 items max) was the only mobile nav and many destinations were
  ...
  return (
    <>
      {/* ── Sidebar (desktop fixed; mobile drawer toggled by hamburger) ───── */}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="lg:hidden flex items-center gap-3 px-4 py-3 border-b border-border bg-surface shrink-0">
          {/* Hamburger — opens the slide-in sidebar drawer. Without this the
              drawer code in Sidebar was unreachable. */}
          <button
            type="button"
            onClick={() => setMobileNavOpen(true)}
            aria-label="Open navigation menu"
            aria-expanded={mobileNavOpen}
            className="p-2 -ml-2 rounded-lg text-textMuted hover:text-white hover:bg-white/10 transition-colors"
          >
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <span className={`w-2 h-2 rounded-full ${connected ? 'bg-green-400 animate-pulse' : 'bg-red-500'}`} aria-hidden />
            <span className="text-xs text-textMuted" role="status">{connected ? 'Live' : 'Offline'}</span>
          </div>
        </header>
        ...
        {/* Mobile bottom tab bar — duplicated from sidebar for one-tap access
            to the most common destinations. The hamburger above opens the
            full nav for the rest. */}
        <nav className="lg:hidden fixed bottom-0 inset-x-0 z-30 flex items-stretch border-t border-border bg-surface/95 backdrop-blur-md" aria-label="Quick navigation">
          {nav.map(({ href, icon: Icon, label }) => {
            ...
            return (
              <Link
                ...
                className={`flex-1 flex flex-col items-center justify-center gap-0.5 py-3 text-xxs font-medium transition-colors ${
