...
export type WsMessage =
  | { event: 'job:queued';        data: Job;                                                                  timestamp: number }
  | { event: 'job:paused';        data: Job;                                                                  timestamp: number }
  | { event: 'job:progress';      data: { jobId: string; workerId?: string; progress?: number; fps?: number; eta?: number; phase?: string; status?: string; workerName?: string; sizeBefore?: number; sizeAfter?: number; error?: string }; timestamp: number }
  | { event: 'job:complete';      data: { jobId: string; sizeBefore?: number; sizeAfter?: number; fileName?: string }; timestamp: number }
  | { event: 'job:failed';        data: { jobId: string; error?: string; fileName?: string };                timestamp: number }
  | { event: 'job:removed';       data: { id: string };                                                       timestamp: number }
  | { event: 'job:cleared';       data: Record<string, never>;                                                timestamp: number }
  | { event: 'worker:discovered'; data: WorkerInfo;                                                           timestamp: number }
  | { event: 'worker:accepted';   data: WorkerInfo;                                                           timestamp: number }
  | { event: 'worker:updated';    data: WorkerInfo;                                                           timestamp: number }
  | { event: 'worker:offline';    data: { id: string; name?: string };                                        timestamp: number }
  | { event: 'worker:progress';   data: { workerId: string; progress: number; fps?: number; eta?: number; phase?: string }; timestamp: number }
  | { event: 'worker:stats';      data: { workerId: string; gpuStats?: GpuStats };                            timestamp: number }
  | { event: 'scan:summary';      data: { sessionId?: string; dir: string; recipe: string; enqueued: number; skipped: number; total: number; alreadyActive: number; error?: string; message: string }; timestamp: number }
  | { event: 'scan:progress';     data: { sessionId: string; dir: string; checked: number; queued: number; skipped: number }; timestamp: number }
  | { event: 'stats:update';      data: DashboardStats;                                                       timestamp: number }
  | { event: 'system:warning';    data: { message: string };                                                  timestamp: number };
