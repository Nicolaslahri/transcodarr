...
//
// Encapsulates the correct per-encoder flags for each quality tier so
// individual recipe cases don't need to repeat the if/else chain.
//
// H.265 tiers:  archive (CRF 18) · high (CRF 20) · balanced (CRF 24) · fast (CRF 26)
// H.264 tiers:  high (CRF 20) · balanced (CRF 23) · fast (CRF 25)
// AV1 tiers:    high (CRF 26) · balanced (CRF 30)
//
// NVENC uses VBR + CQ mode (-rc vbr -cq) which gives better quality than
// fixed-QP mode by allowing bitrate headroom on complex scenes.
// AMD AMF uses -qp with a quality preset string.
// Intel QSV uses -global_quality with a named preset.

type H265Tier = 'archive' | 'high' | 'balanced' | 'fast';
type H264Tier = 'high' | 'balanced' | 'fast';
type AV1Tier  = 'high' | 'balanced';

function encodeH265(enc: string, tier: H265Tier): string[] {
  const crf: Record<H265Tier, string> = { archive: '18', high: '20', balanced: '24', fast: '26' };
  const q = crf[tier];
...
function encodeH264(enc: string, tier: H264Tier): string[] {
  const crf: Record<H264Tier, string> = { high: '20', balanced: '23', fast: '25' };
  const q = crf[tier];
...
function encodeAV1(enc: string, tier: AV1Tier): string[] {
  const crf: Record<AV1Tier, string> = { high: '26', balanced: '30' };
  const q = crf[tier];
